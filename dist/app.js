...
// src/app-runner.ts
import { chmodSync, existsSync as existsSync5, mkdtempSync, mkdirSync as mkdirSync4, rmSync, writeFileSync as writeFileSync5 } from "fs";
...
function updateChangelog(cwd, release) {
  ...
  const existing = existsSync3(path) ? readFileSync4(path, "utf8") : "";
...
// src/output.ts
import { appendFileSync, existsSync as existsSync4, lstatSync, mkdirSync as mkdirSync2, readFileSync as readFileSync6, unlinkSync, writeFileSync as writeFileSync4 } from "fs";
...
function getReleaseOutputPaths(cwd, outputDir = ".hooversion") {
  ...
  if (existsSync4(outputsPath) && lstatSync(outputsPath).isFile()) {
    try {
      const payload = JSON.parse(readFileSync6(outputsPath, "utf8"));
...
function createGitAuthArtifacts(workDir, token) {
  const tokenPath = join7(workDir, ".git-token");
  const askpassPath = join7(workDir, ".git-askpass");
  const cleanup = () => {
    rmSync(tokenPath, { force: true });
    rmSync(askpassPath, { force: true });
  };
  try {
    writeFileSync5(tokenPath, `${token}
`, { encoding: "utf8", mode: 384 });
    chmodSync(tokenPath, 384);
    writeFileSync5(askpassPath, `#!/bin/sh
case "$1" in
...
function installProjectDependencies(repoDir, configuredCommand, secret) {
  const command = configuredCommand ?? (existsSync5(join7(repoDir, "bun.lock")) ? "bun install --frozen-lockfile" : "");
  if (!command)
    return;
