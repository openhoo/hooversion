...
function writeGitHubWorkflow(cwd, options = {}) {
  const path = join7(cwd, ".github", "workflows", "hooversion.yml");
  mkdirSync4(join7(cwd, ".github", "workflows"), { recursive: true });
  writeFileSync5(path, renderGitHubWorkflow(options));
  return path;
}
function renderGitHubWorkflow(options = {}) {
...
async function initCommand(cwd, flags) {
  ...
  const workflowPath = flags.booleans.has("no-workflow") ? undefined : writeGitHubWorkflow(cwd, {
